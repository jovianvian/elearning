@extends('layouts.app', ['title' => __('ui.courses')])

@section('content')
<div x-data="courseListPage()" data-async-list data-fragment="#courses-table-fragment">
...
<x-ui.table-toolbar :search-value="request('q')" :search-placeholder="__('ui.search_course_subject_class')">
    <x-slot:filters>
        <div>
                    ...
                    <td>
                        <div class="inline-flex items-center gap-2">
                            ...
                            @if(auth()->user()->hasRole('super_admin','admin'))
                                ...
                                <button type="button" class="tera-btn tera-btn-danger !px-3 !py-1.5" @click="destroyCourse({{ $course->id }}, @js($course->title))">{{ __('ui.delete') }}</button>
                            @endif
                        </div>
                    </td>
                </tr>
...
function courseListPage() {
    return {
        async destroyCourse(id, title) {
            ...
            try {
                const { response, payload } = await window.Teramia.fetchJson(`/courses/${id}`, {
                    method: 'POST',
                    body: JSON.stringify({ _method: 'DELETE' }),
                });
                if (!response.ok) throw new Error(payload?.message || @js(__('ui.failed_delete_course')));
                await window.Teramia.toast('success', payload.message || @js(__('ui.course_moved_to_trash')));
                await window.Teramia.refreshFragment(window.location.href, '#courses-table-fragment');
            } catch (error) {
                window.Teramia.toast('error', error.message || @js(__('ui.failed_delete_course')));
            }
        }
    };
}
