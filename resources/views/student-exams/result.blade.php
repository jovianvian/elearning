...
@section('content')
    <div class="bg-white rounded-xl border border-slate-200 p-6">
        <h2 class="text-xl font-semibold">Exam Result</h2>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4">
                ...
                <div class="mt-2 text-sm">
                    @if($question?->type === 'multiple_choice')
                        <div>Your answer: {{ $answer->selectedOption?->option_key ?? '-' }}</div>
                    @else
                        <div>Your answer: {{ $answer->answer_text ?: '-' }}</div>
                    @endif
                    ...
                    <div class="mt-1">Score: {{ $answer->score }}</div>
                    @if($answer->teacher_feedback)
                        <div class="mt-1 text-slate-600">Feedback: {{ $answer->teacher_feedback }}</div>
                    @endif
                </div>
            </div>
        @endforeach
    ...
    <a href="{{ route('student-exams.index') }}" class="inline-block px-4 py-2 border rounded-lg text-sm">Back to My Exams</a>
@endsection
